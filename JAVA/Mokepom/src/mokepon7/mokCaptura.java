...
import mokepon7bien.objetos.Objeto;
...
public class mokCaptura extends mokepon {
	...
	public void Atacar(mokepon atacado, int numAtak) {
		if (!this.isDebilitado()) {
			if (this.getSetAtks().get(numAtak).getUseAct() > 0) {
				double damage = (((((2 * this.getLevel()) / 5) + 2) * (this.getSetAtks().get(numAtak).getPower())
						* (this.getAtk() / atacado.getDef()) / 50 + 2)
						* (super.efectivitat(this.getSetAtks().get(numAtak).getTipo(), atacado.getTipo())));
				this.getSetAtks().get(numAtak).setUseAct(this.getSetAtks().get(numAtak).getUseAct() - 1);
	...
	public Objeto getObjetoP() {
		return objetoP;
	}

	public void setObjetoP(Objeto objetoP) {
