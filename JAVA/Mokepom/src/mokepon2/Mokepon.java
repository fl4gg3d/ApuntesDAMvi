...
public class Mokepon {
	...
	public Mokepon() {
		...
		this.hp_max = 10;
		this.hp_act = hp_max;
		this.tipo = TipusMokepon.normal;
		this.debilitado = false;

	}

	public Mokepon(String nom, TipusMokepon tipus) {
    ...
    public MokeponCapturao capturar(String nomEntrenador, String nomDonat) {  
        //si nosaltres mateixos no som de classe MokeponCapturat
        if(!(this instanceof MokeponCapturao)) {
        	MokeponCapturao moke = new MokeponCapturao(this, nomDonat, nomEntrenador);
        	return moke;
            //capturar el Mokepon. Això ho heu de fer vosaltres
            //Has de crear un MokeponCapturat fent servir el propi Mokepon. potser algun dels constructors de MokeponCapturat et serveix…
        }else{
