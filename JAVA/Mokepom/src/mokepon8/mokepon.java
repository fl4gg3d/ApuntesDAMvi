...
public class mokepon implements Comparable<mokepon>{
	...
	// Registra 2 ataques a la vez, en caso de que se haya asignado algun ataque
	// asigna el ultimo;
	...
	public double efectivitat(Tipos atac, Tipos defensa) {
		if (atac == Tipos.fuego && defensa == Tipos.agua || atac == Tipos.agua && defensa == Tipos.planta
				|| atac == Tipos.planta && defensa == Tipos.fuego) {
			return 0.5;
		} else if (atac == Tipos.agua && defensa == Tipos.fuego || atac == Tipos.fuego && defensa == Tipos.planta
		...
		if (!this.debilitado) {
			if (!atacado.debilitado) {
				if (this.setAtks.get(numAtak).getUseAct() > 0) {
					double damage = (((((2 * this.level) / 5) + 2) * (this.setAtks.get(numAtak).getPower())
							* (this.atk / atacado.def) / 50 + 2)
							* (efectivitat(this.setAtks.get(numAtak).getTipo(), atacado.tipo)));
	...
	public Huevo reproduccion(mokepon moke1, mokepon moke2) throws Exception {
	    assert moke1 != null : "El primer mokepon no existe";
	    assert moke2 != null : "El segundo mokepon no existe";

	    assert moke1.tipo == moke2.tipo : "Los mokepons son de tipos diferentes";

	    assert moke1.sexo != moke2.sexo : "Los pokemons son del mismo sexo";

	    assert !moke1.debilitado && !moke2.debilitado :
	            "Alguno de los mokepons está debilitado\n" +
	            moke1.nombre + ": " + moke1.debilitado + "\n" +
